...
;( function( $, window, undefined ) {
    ...
    $.ReviewSlider = function( options, element ) {
        this.$el = $( element );
        this._init( options );
    };

    // the options
    $.ReviewSlider.defaults = {
    ...
    $.ReviewSlider.prototype = {
        _init : function( options ) {

            // options
            this.options = $.extend( true, {}, $.ReviewSlider.defaults, options );
            // cache some elements and initialize some variables
            this._config();
            // show current item
            this.$items.eq( this.current ).addClass( 'review-unit-current' );
        ...
        _config : function() {
            // the content items
            this.$items = this.$el.children( 'div.review-unit' );
            // total items
            ...
            if( this.support ) {
                this.$progress = $('<div class="review-timer"></div>').appendTo( this.$el );
            }
        },
        ...
        _next : function() {
            ...
            // hide previous item
            this.$items.eq( this.current ).removeClass( 'review-unit-current' );
        ...
        destroy : function() {
            ...
            this.$items.removeClass( 'review-unit-current' ).css( {
                'position' : 'relative',
                'z-index' : 100,
                'pointer-events' : 'auto',
                'opacity' : 1
    ...
    $.fn.reviewSlider = function( options ) {
        if ( typeof options === 'string' ) {
            var args = Array.prototype.slice.call( arguments, 1 );
            this.each(function() {
                var instance = $.data( this, 'reviewSlider' );
                if ( !instance ) {
                    logError( "cannot call methods on reviewSlider prior to initialization; " +
                    "attempted to call method '" + options + "'" );
                    return;
                }
                if ( !$.isFunction( instance[options] ) || options.charAt(0) === "_" ) {
                    logError( "no such method '" + options + "' for reviewSlider instance" );
                    return;
                }
            ...
            this.each(function() {
                ...
                if ( instance ) {
                    ...
                    instance = $.data( this, 'reviewSlider', new $.ReviewSlider( options, this ) );
                }
            });
        }
        return this;
...
/*! lightgallery - v1.2.17 - 2016-04-11
...
$(function() {
    ...
    $('.fullheight-section').css('height', wheight);
    ...
    $(window).scroll(function() {
        ...
        if (windowpos > $('#ourservices').offset().top) {
            $('nav li a').removeClass('active');
            $('a[href$="#ourservices"]').addClass('active');
        }
        if (windowpos > $('#ourwork').offset().top) {
            $('nav li a').removeClass('active');
            $('a[href$="#ourwork"]').addClass('active');
        }
    ...
    // Tween to scale heart
    var heart_tween = TweenMax.to('#anchor-svg', 0.8, {
        transform: 'scale(0.9)',
        ease: Elastic.easeOut.config(1, 0.6, true),
    ...
    // PINNED NAVIGATION
    // Controller for Pinned Nav
    var navCtrl = new ScrollMagic.Controller({
        globalSceneOptions: {
            triggerHook: 'onLeave'
        }
    });
    ...
    var navScene = new ScrollMagic.Scene ({
        triggerElement: '#tabs', // point of execution
        duration: 0
    })
    .setPin('#tabs') // the element we want to pin
    .addTo(navCtrl);


    // INTRO PARALLAX
    // Controller for #intro Parallax
    var coverCtrl = new ScrollMagic.Controller({
        globalSceneOptions: {
            triggerHook: 'onLeave',
            duration: '200%'
        }
    });
    // Init Parallax for #intro
    var introScene = new ScrollMagic.Scene ({
        triggerElement: "#parallax1",
    })
    .setTween('#parallax1 > div', {y: '100%', ease: Linear.easeNone, overwrite: false})
    .addTo(coverCtrl);
    ...
    //Init Parallax for #welcome
    var welcomeScene = new ScrollMagic.Scene ({
        triggerElement: "#parallax2",
    })
    .setTween('#parallax2 > .parallaxChild', {y: '200%', ease: Linear.easeNone, overwrite: false, immediateRender: false})
    .addTo(parallaxCtrl);


    // Init Parallax for #reviews
    var reviewScene = new ScrollMagic.Scene({
        triggerElement: "#parallax3"
    })
    .setTween("#parallax3 > .parallaxChild", {y: "180%", ease: Linear.easeNone, overwrite: false, immediateRender: false})
    .addTo(parallaxCtrl);


    // variable to store mouse position
    var mouseTopPerc = 0;
    // function to be used to store mouse position
    ...
    //init Controller
    var footerCtrl = new ScrollMagic.Controller();

    // build tween
    var footerlogotween = TweenMax.to('#footerlogo', 0.9, {rotationX: 360});

    //build scene and supply getMousePos function as duration
    var escena = new ScrollMagic.Scene({
        triggerElement: '.footer-logo-container',
        duration: mouseTopPerc
    })
    .setTween(footerlogotween)
    .addTo(footerCtrl);
    ...
    var $hairstylist = $("path#hairstylist");
    var $gi = $("path#gi");
    var $n = $("path#n");
    var $a = $("path#a");
    ...
    pathPrepare($n);
    pathPrepare($a);
    pathPrepare($hairstylist);


    // init controller
    var svgCtrl = new ScrollMagic.Controller({
        globalSceneOptions: {
            triggerHook: 'onLeave'
        }
    });
    ...
    .add(TweenMax.to("#svgContainer", 0.5, {backgroundColor: "rgba(255, 255, 255, 0.9)", ease:Linear.easeNone, overwrite: false, immediateRender: false }))
    .add(TweenMax.to(".kicker-text", 1.2, {opacity:1, alpha: 0.7, overwrite: false, immediateRender: false}))
    .add(TweenMax.to([$gi, $n, $a, $hairstylist], 3, {strokeDashoffset: 0, ease:Power1.easeInOut, y:1, overwrite: false, immediateRender: false})) // draw word for 0.9
    .add(TweenMax.to([$gi, $n, $a], 2.5, {stroke: "#da251c", ease:Linear.easeNone, overwrite: false, immediateRender: false}), 0) // change color during the whole thing
    .add(TweenMax.to($hairstylist, 2.5, {stroke: "#9d9d9d", ease:Linear.easeNone, overwrite: false, immediateRender: false}), 0) // change color during the whole thing
    .add(TweenMax.to($hairstylist, 0.5, {alpha: "0.7", fill:"#9d9d9d", overwrite: false, immediateRender: false})) // change color during the whole thing
    .add(TweenMax.to([$gi, $n, $a], 1, {alpha: "0.9", fill: "#da251c", overwrite: false, immediateRender: false})) // change color during the whole thing
    ...
    // Use to free up resources. Init destroy method
    //$('div#swoosh').on('click', function(e) {
    //    e.preventDefault();
    //    if ($(this).hasClass("disabled")) {
    //        alert("Already removed. Please reload the page and try again");
    //    } else {
    ...
    // Review slider set up
    /*
    - how to call the plugin:
    $( selector ).reviewSlider( [options] );
    - options:
    {
        // default transition speed (ms)
        speed : 700,
    ...
    - destroy:
    $( selector ).reviewSlider( 'destroy' );
    */
    $('#review-slider').reviewSlider();


    //Modal Morph-button setup
    ...
    var el = document.querySelector( '.morph-button' ); // grab all el that--> div.morph-button

    [].slice.call(document.querySelectorAll('.morph-button')).forEach(function (buttoned) {
        new UIMorphingButton(buttoned, {
            closeEl : '.icon-circle-down',
            ...
            onAfterOpen : function() {
                ...
                classie.addClass( buttoned, 'scroll' );
            },
            onBeforeClose : function() {
                // remove class "noscroll" to body
                classie.removeClass( document.body, 'noscroll' );
                // remove scroll class from main el
                classie.removeClass( buttoned, 'scroll' );
                // don't allow to scroll
                noScroll();
            },
     ...
     */
    /*
    var $items  = $('<div class="sl-slide sl-slide-color-2" data-orientation="horizontal" data-slice1-rotation="-5"
    data-slice2-rotation="10" data-slice1-scale="2" data-slice2-scale="1"><div class="sl-slide-inner bg-1">
    <div class="sl-deco" data-icon="t"></div><h2>some text</h2><blockquote><p>bla bla</p><cite>Margi Clarke</cite>
    </blockquote></div></div>');
